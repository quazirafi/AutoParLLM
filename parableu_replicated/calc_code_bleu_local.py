...
def getCodeBLEUCustomKeyWords(hypothesis,references,topKSelect,keywords,lang):
    # lang = args.lang
    ...
    # dictVocab=getImportantWords(references)
    # keywords = list(dictVocab.keys())[:topKSelect]
...
def getCodeBLEU(hypothesis,references,lang='c'):
    # lang = args.lang
    # alpha,beta,gamma,theta = [float(x) for x in args.params.split(',')]
    #
    # # preprocess inputs
...
def getParaBLEU(hypothesis,references,lang='ruby'):
    # lang = args.lang
    # alpha,beta,gamma,theta = [float(x) for x in args.params.split(',')]
    #
    # # preprocess inputs
    ...
    # calculate weighted ngram match
    keywords = [x.strip() for x in open('keywords/parableu.txt', 'r', encoding='utf-8').readlines()]
    def make_weights(reference_tokens, key_word_list):
        return {token:1 if token in key_word_list else 0.2 \
                for token in reference_tokens}
    tokenized_refs_with_weights = [[[reference_tokens, make_weights(reference_tokens, keywords)]\
